...
if not defined _OS_BUILD (
    ...
    echo.
    echo %orange%╔══════════════════════════════════════════════════════════════════════════════╗
    echo ║                   COULD NOT VERIFY WINDOWS VERSION                           ║
    echo ╚══════════════════════════════════════════════════════════════════════════════╝%u%
    echo.
    echo %orange%  Batlez Tweaks could not determine your Windows build number.%u%
    echo %c%  This tool is built for Windows 11 ^(build 22000 or later^). If you%u%
    echo %c%  are on Windows 10, some tweaks may not apply correctly or may fail.%u%
    echo.
    choice /C CP /M "Press C to Cancel or P to Proceed anyway"
    if errorlevel 2 goto SkipBuildCheck
    if errorlevel 1 goto Destruct
)
...
chcp 437 >nul
powershell -Command "try { $response = Invoke-RestMethod -Uri 'https://api.github.com/repos/Batlez/Batlez-Tweaks/releases/latest' -Headers @{'User-Agent'='Batlez-Tweaks-Updater'}; $version = $response.tag_name -replace '^v', ''; $name = $response.name; if ($name -match 'Batlez V(.+)') { $nameVersion = $matches[1] } else { $nameVersion = $version }; $fileUrl = $null; $asset = $response.assets | Where-Object { $_.name -like '*.bat' } | Select-Object -First 1; if ($asset) { $fileUrl = $asset.browser_download_url } else { try { $contents = Invoke-RestMethod -Uri (\"https://api.github.com/repos/Batlez/Batlez-Tweaks/contents/?ref=$($response.tag_name)\") -Headers @{'User-Agent'='Batlez-Tweaks-Updater'}; $batFile = $contents | Where-Object { $_.name -like '*.bat' } | Select-Object -First 1; if ($batFile) { $fileUrl = $batFile.download_url } } catch { } }; Write-Host \"VERSION=$nameVersion\"; Write-Host \"URL=$($response.html_url)\"; if ($fileUrl) { Write-Host \"FILEURL=$fileUrl\" } } catch { Write-Host 'ERROR=Unable to check for updates' }" > "%temp%\update_check.txt" 2>nul
chcp 65001 >nul
...
if defined latest_version (
    ...
    if "!update_available!"=="true" (
        ...
        if not defined file_url (
            echo.
            echo Couldn't find a direct download link for this release.
            echo Opening your browser to the release page instead...
            echo.
            start "" "!download_url!"
            echo.
        ...
        if not exist "!DownloadsFolder!" set "DownloadsFolder=%USERPROFILE%\Desktop"

        set "NewFileName=Batlez Tweaks v!latest_version!.bat"
        set "NewFilePath=!DownloadsFolder!\!NewFileName!"

        echo.
        echo Downloading Batlez Tweaks v!latest_version! to:
        echo   !NewFilePath!
        echo.
		chcp 437 >nul
        powershell -Command "try { Invoke-WebRequest -Uri '!file_url!' -OutFile '!NewFilePath!' -UseBasicParsing -ErrorAction Stop; Write-Host 'Saved successfully.' } catch { Write-Host ('Download failed: ' + $_.Exception.Message) }"
		chcp 65001 >nul
		
        set "DLSize=0"
        if exist "!NewFilePath!" for %%F in ("!NewFilePath!") do set "DLSize=%%~zF"

        if !DLSize! GTR 50000 (
            echo Download complete.
            echo.
            echo Launching the new version now - this window will close.
            timeout /t 2 >nul
            start "" "!NewFilePath!"
            goto Destruct
        ) else (
            echo.
            echo Download failed or the file looked incomplete.
            echo Opening your browser to the release page instead...
            echo.
            del "!NewFilePath!" >nul 2>&1
            start "" "!download_url!"
            echo.
            echo Press any key to continue with current version for now...
            pause >nul
